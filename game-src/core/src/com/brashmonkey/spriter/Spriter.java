...
//				drawer = drawerClass.getDeclaredConstructor(drawerTypes).newInstance(drawerDependencies);
		...
		throw new RuntimeException(GwtChange.removed());
	}
...
//			e.printStackTrace();
//		}
		new RuntimeException(GwtChange.removed());
	}
	...
	public static void load(InputStream stream, String scmlFile){
		...
		if(false) {
			try {
				Loader loader = null;//loaderClass.getDeclaredConstructor(loaderTypes).newInstance(loaderDependencies);
	...
	public static Player newPlayer(String scmlFile, int entityIndex, Class<? extends Player> playerClass){
		...
		if(false) {
			try {
				Player player = null;//playerClass.getDeclaredConstructor(Entity.class).newInstance(loadedData.get(scmlFile).getEntity(entityIndex));
				players.add(player);
				return player;
			} catch (Exception e) {
				e.printStackTrace();
