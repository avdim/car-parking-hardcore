...
public class LayerSystem extends IteratingSystemWithSkips {
	...
	public LayerSystem() {
		super(Family.all(CompositeTransformComponent.class).get(), App.params.performanceFixes() ? 60 : 0);
	...
	@Override
	protected void processEntity2(Entity entity, float deltaTime) {//49%
		NodeComponent nodeComponent = nodeMapper.get(entity);
		LayerMapComponent layerMapComponent = layerMapper.get(entity);
		updateLayers(nodeComponent.children, layerMapComponent);//37%
